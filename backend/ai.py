...
- Output ONLY the tailored Education section in clean plain text, ready to paste into a resume
...
- Keep the output concise: no more than 5 categories, no more than 6 items per category
- Output ONLY the tailored Skills section in clean plain text, ready to paste into a resume
...
1. CONTACT INFO — name, email, phone, location, LinkedIn, portfolio/GitHub (if applicable)
2. PROFESSIONAL SUMMARY — a 2–3 sentence pitch that positions the candidate for this specific role
...
- Lead with the candidate's years of experience and core identity (e.g. "Full-stack engineer with 5+ years...")
...
- Output ONLY the tailored Profile section in clean plain text, ready to paste into a resume
...
INSTRUCTIONS:
- Rewrite bullet points to emphasize responsibilities and achievements most relevant to this role
- Lead each bullet with a strong action verb (e.g. "Engineered," "Led," "Reduced," "Launched")
- Quantify impact wherever the data is available (e.g. "reduced load time by 40%")
- Remove or compress bullets that are irrelevant to this role
- Keep each bullet to one line (max ~120 characters)
- Preserve all employer names, job titles, locations, and dates exactly as provided — do not alter facts
- Do not invent metrics or responsibilities that weren't in the original
- Maintain reverse-chronological order
- Output ONLY the tailored Experience section in clean plain text, ready to paste into a resume
...
def fmt_skills(rows: list[Skill]) -> str:
    return "\n".join(
        f"- {s.name} | category: {s.category or 'none'} | proficiency: {s.proficiency or 'unspecified'}"
        for s in rows
    )
...
def create_resume(
    ...
    if not profile:
        raise HTTPException(status_code=422, detail="Profile not found. Please complete your profile before generating a resume.")
    experience = db.query(Experience).filter(Experience.owner_id == user_id).all()
    if not experience:
        raise HTTPException(status_code=422, detail="No experience entries found. Please add experience before generating a resume.")
    skills = db.query(Skill).filter(Skill.owner_id == user_id).all()
    if not skills:
        raise HTTPException(status_code=422, detail="No skills found. Please add skills before generating a resume.")
    education = db.query(Education).filter(Education.owner_id == user_id).all()
    if not education:
        raise HTTPException(status_code=422, detail="No education found. Please add education before generating a resume")

    position_info = body.position_info

    with ThreadPoolExecutor(max_workers=4) as executor:
        future_education  = executor.submit(call_gemini, EDUCATION_PROMPT.format(
            education=fmt_education(education), position_info=position_info),
            list[TailoredEducation]
        )
        future_experience = executor.submit(call_gemini, EXPERIENCE_PROMPT.format(
            experience=fmt_experience(experience), position_info=position_info),
            list[TailoredExperience]
        )
        future_skills     = executor.submit(call_gemini, SKILLS_PROMPT.format(
            skills=fmt_skills(skills), position_info=position_info),
            list[TailoredSkill]
        )
        future_profile    = executor.submit(call_gemini, PROFILE_PROMPT.format(
